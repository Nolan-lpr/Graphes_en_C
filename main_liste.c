    ...
    while (i < argc) {
        if (strcmp(argv[i], "-i") == 0) {
            if (i + 1 >= argc) {
                ...
                printf("Arguments :\n");
                printf("  -i <fichier_graphe>      : le fichier graphe à lire\n");
                printf("  -o <fichier_dot>         : le fichier dot à produire\n");
                printf("  -start <sommet_depart>   : le sommet de depart\n");
                printf("  -goal <sommet_arrivee>   : le sommet à atteindre\n");
                return 1;
            }
            in = fopen(argv[i + 1], "r");
            if (!in) { 
...
t_list list_add_head(t_vertex e, t_list l) {
    ...
    n->p_next = l;     // Pointer vers l'ancienne tete
    return n;          // Nouvelle tete
}
...
// Retire le premier maillon de la liste l et retourne la nouvelle tete
...
// Remarque : la fin d'une liste (NULL) est la meme chose qu'une liste vide !
...
///// lecture d'un graphe sur un fichier //// 
t_graph_list * read_graph_list(FILE *in) {
    int n, u, v;
    char line[MAX_READ_LINE]; // Buffer pour lire une ligne entiere
    ...
    n = atoi(line); // Convertir la ligne en entier
    t_graph_list *g = graph_list_new(n);

    // Lire les arcs ligne par ligne
    while (fgets(line, MAX_READ_LINE, in) != NULL) {
        if (sscanf(line, "%d %d", &u, &v) == 2) {
            graph_list_add_arc(g, u, v);
        }
    }
    return g;
}
...
////Export des graphes au format DOT //////
void graph_to_dot_list(t_graph_list *g, FILE *out) {
    fprintf(out, "digraph G {\n");
    for (int i = 0; i < g->size; i++) {
        t_node *lc = list_cursor_new(g->adj[i]);
        while (!list_cursor_at_end(lc)) {
            fprintf(out, "  %d -> %d;\n", i, list_cursor_get_val(lc));
...
/* Inverser tous les arcs du graphe pour identifier les suffixes, puis les prefixes lors de Kosaraju*/
t_graph_list * graph_inverse(t_graph_list *g) {
    t_graph_list *gi = graph_list_new(g->size);
    for (int i = 0; i < g->size; i++) {
        t_node *lc = list_cursor_new(g->adj[i]);
        while (!list_cursor_at_end(lc)) {
            graph_list_add_arc(gi, list_cursor_get_val(lc), i); // inverser
            lc = list_cursor_next(lc);
        }
    }
    return gi;
...
void Parcours(t_graph_list *g, t_vertex x, t_bool *marking) {
    if (marking[x]) return;

    marking[x] = TRUE;
    // traitement prefixe (avant les successeurs)
    #ifdef DEBUG
    printf("%d ", x);
    #endif

    t_node *lc = list_cursor_new(g->adj[x]);
    while (!list_cursor_at_end(lc)) {
        Parcours(g, list_cursor_get_val(lc), marking);
        lc = list_cursor_next(lc);
    }
    // traitement suffixe (apres les successeurs)
}
...
void Recherche_recur(t_graph_list *g, t_vertex x, t_vertex y) {
    ...
    int found = Recherche_recur_f(g, x, y, marking, stack);
    
    #ifdef DEBUG
    if (found) {
        printf("Chemin trouve : ");
        
        // Afficher directement en depilant (sans inverser)
...
//Fonction qui effectue le parcours en profondeur à partir d’un sommet donne
//si le marquage indique qu’il n’a pas encore ete traverse
//et se rappelle sur chaque successeur du sommet.
//Elle s’arrete quand elle arrive sur le sommet cible (succes), ou quand elle a visite
//tous les sommets atteignables (echec)
int Recherche_recur_f(t_graph_list *g, t_vertex x, t_vertex y, t_bool *marking, t_stack *stack) {
    ...
    if (marking[x] == FALSE) {
        ...
        t_node *lc = list_cursor_new(g->adj[x]);
        while (!list_cursor_at_end(lc)) {
            t_vertex w = list_cursor_get_val(lc);
...
//Fonction enum
// etape 1 : Faire un parcours en profondeur (DFS) sur le graphe original g pour calculer l’ordre suffixe.
// etape 2 : Faire un DFS sur le graphe inverse en suivant l’ordre inverse du suffixe pour identifier les composantes fortement connexes.
void enum_cfc_kosaraju(t_graph_list *g) {
    ...
    Kosaraju_1(g, marking, order);

    // 2) Graphe inverse
    t_graph_list *gi = graph_inverse(g);
...
//Premier parcours recursif de l'algorithme de Kosaraju (fonction globale)
...
//Appels recursifs pour le premier parcours de Kosaraju
int Kosaraju_1_recur(t_graph_list *g, t_vertex x, t_bool *marking, int *order, int step) {
    if (marking[x] == FALSE) {
        ...
        t_node *cursor = list_cursor_new((g->adj)[x]);
        t_vertex y;        
        while (!list_cursor_at_end(cursor)) {
            y = list_cursor_get_val(cursor);
            step = Kosaraju_1_recur(g, y, marking, order, step);
            cursor = list_cursor_next(cursor);
        }
        order[x] = step;
        step++;
    }
...
//Deuxieme parcours recursif de l'algorithme de Kosaraju (fonction globale)
